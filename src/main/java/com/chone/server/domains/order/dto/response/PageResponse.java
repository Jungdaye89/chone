...
public record PageResponse<T>(
    @Schema(
            description = "페이지 컨텐츠",
            required = true,
            subTypes = {OrderPageResponse.class})
        List<T> content,
  ...
  private record PageContent(
      ...
      @Schema(description = "페이지 크기", example = "20", required = true) int size,
      @Schema(description = "전체 항목 수", example = "100", required = true) long totalElements,
      @Schema(description = "전체 페이지 수", example = "5", required = true) int totalPages,
